...
<script>
    ...
    $(document).ready(function()
    {
        ...
        elseif($model->getLevel($model->id)==1)
        {?>
        var arc = d3.svg.arc()
            .innerRadius(30)
            .outerRadius(40)
        ...
        elseif($model->getLevel($model->id)==2){?>
        var arc = d3.svg.arc()
            .innerRadius(30)
            .outerRadius(40)
            .startAngle(0 * (pi/180))
...
<script>
    $(document).ready(function(){
        window.randomize = function() {
            <?php
            if($model->findByPk($model->id)->level==0)
            {

            }elseif($model->findByPk($model->id)->level==1)
            {
            ?>
            $('.radial-progress').attr('data-progress', Math.floor(0.33*100));
            <?php
            }elseif($model->findByPk($model->id)->level==2)
            {
            ?>
                    ...
                    <div class="block-height">
                        ...
                        <div class="block-table-style">
                            <div class="display-table-cell info-black points-style">
                                ...
                                if(isset($model->id))
                                {
                                    echo (int)User::model()->findByPk($model->id)->points;
                                }
                                ?>
                            </div>
                        </div>
                ...
                <div>
                    ...
                    else
                    {
                        ...
                        <div class='new-comment position-relative' style="padding: 7px;  border-bottom: 1px solid #eaeaea;margin-bottom: 7px;">
                            <?php echo $form->hiddenField($comment_m,'commented_user_id',array("value"=>$model->id));?>
...
//                                var fd =$(this).serializeArray();
    ...
    <?php else: ?>
        <div class="">
            <div class="quadr">
                <div class="main-block-padding">
                    <img src="<?= Yii::app()->request->baseUrl; ?>/img/incognito.png"/>
                    <div></div>
                    <div style="font-size:24px;font-weight:bold;">Information was hidden</div>
                    <div style="color:#A99C9C;font-size:15px">Please add user to friends list</div>
                </div>
            </div>
        </div>
    <?php endif; ?>
